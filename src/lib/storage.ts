...
export async function loadCampaignInfo(campaignId: string): Promise<CampaignInfo | null> {
  const campaignDict = await getDataFromDb(`campaignInfos/${campaignId}`, null);
  if (campaignDict === null) return null;
  const [campaign] = Object.values(campaignDict) as CampaignInfo[];
  return campaign;
}
